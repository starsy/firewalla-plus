...
exports.getLicense = function(luid, mac, callback) {
  ...
  boneAPITimeSeries.recordHit('license')
    .exec()
    .catch(err => log.error("Error when record hit 'license'", err));
...
exports.checkin = function(config, license, info, callback) {
  ...
  boneAPITimeSeries.recordHit('checkin')
    .exec()
    .catch(err => log.error("Error when record hit 'checkin'", err));
...
exports.device = function(cmd, obj, callback) {
  ...
  boneAPITimeSeries.recordHit('device')
    .exec()
    .catch(err => log.error("Error when record hit 'device'", err));
...
exports.log = function(cmd, obj, callback) {
  ...
  boneAPITimeSeries.recordHit('log')
    .exec()
    .catch(err => log.error("Error when record hit 'log'", err));
...
exports.intel = function(ip, type, action, intel, callback) {
  ...
  log.debug("Request:", JSON.stringify(intel, null, '  '), {});
  boneAPITimeSeries.recordHit('intel')
    .exec()
    .catch(err => log.error("Error when record hit 'intel'", err));
  ...
  request(options, (err, httpResponse, body) => {
    ...
    if (!err && body) {
      obj = body;
      log.debug("Response:", JSON.stringify(obj, null, '  '), {});
    }
    if (callback) {
      callback(err, obj);
    }
...
exports.flowgraph = function(action, obj, callback) {
  ...
  boneAPITimeSeries.recordHit('flowgraph')
    .exec()
    .catch(err => log.error("Error when record hit 'flowgraph'", err));
...
exports.hashset = function(hashsetid, callback) {
  ...
  boneAPITimeSeries.recordHit('hashset')
    .exec()
    .catch(err => log.error("Error when record hit 'hashset'", err));
...
exports.hashsetAsync = Promise.promisify(exports.hashset)
...
exports.getServiceConfig = function(callback) {
  ...
  boneAPITimeSeries.recordHit('getServiceConfig')
    .exec()
    .catch(err => log.error("Error when record hit 'getServiceConfig'", err));
...
let intelFeedback = function(feedback, callback) {
  ...
  boneAPITimeSeries.recordHit('intelFeedback')
    .exec()
    .catch(err => log.error("Error when record hit 'intelFeedback'", err));
...
exports.intelFinger = async (target) => {
  ...
  let options = {
    ...
    maxAttempts: 5,   // (default) try 5 times
    retryDelay: 6000,  // (default) wait for 6s before trying again
  };

  return new Promise((resolve, reject) => 
    rr(options, (err, resp, body) => {
      if (err) {
        log.error("Error while requesting ", err, {});
        resolve();
        return;
      }
      if (!resp) {
      ...
      if (resp.statusCode < 200 || resp.statusCode > 299) {
        log.error("Error HTTP error code", resp.statusCode);
        resolve();
        return;
      }
